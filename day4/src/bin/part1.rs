...
fn process(input: &str) -> impl ToString {
    ...
    let (_, guard, minute) = sleeping_tallies
        .iter()
        .map(|(guard, tallies)| {
            let (total, _, max_minute) = tallies.iter().fold(
                (0, 0, 0),
                |(total, max_tally, max_minute), (&minute, &tally)| {
                    if tally > max_tally {
                        (total + tally, tally, minute)
                    } else {
                        (total + tally, max_tally, max_minute)
                    }
                },
            );
...
mod tests {
    ...
    fn entries() {
        assert_eq!(
            "[1518-11-01 00:00] Guard #10 begins shift".parse::<Entry>(),
            Ok(Entry {
                time: EntryTime {
                    year: 1518,
                    month: 11,
                ...
                action: EntryAction::BEGIN(10)
            })
        );
        assert_eq!(
            "[1518-11-01 00:05] falls asleep".parse::<Entry>(),
            Ok(Entry {
                time: EntryTime {
                    year: 1518,
                    month: 11,
                ...
                action: EntryAction::SLEEP
            })
        );
        assert_eq!(
            "[1518-11-01 00:25] wakes up".parse::<Entry>(),
            Ok(Entry {
                time: EntryTime {
                    year: 1518,
                    month: 11,
